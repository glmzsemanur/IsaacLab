...
class IsaacLabVectorEnv(
    ...
    def step(self, actions: Union[torch.Tensor, F32NDArray]) -> tuple[
        ...
        # Expose Isaac's extras["log"] (reward components, curriculum level, terminations, etc.)
        # as episode_info so train.py's logger picks them up via logger.update_metric().
            ...
            for key, value in isaac_log.items():
                try:
                    if isinstance(value, torch.Tensor):
                        episode_info[key] = float(value.mean().cpu().item())
                    else:
